...
create_subsets <- function(data) {
  D <- cbind(data[, which(colnames(data) == "Race"):(which(colnames(data) == "C_SA")-1)], data[, c("time", "event")])
  M <- cbind(data[, which(colnames(data) == "C_SA"):which(colnames(data) == "RVpowerIndex_S")], data[, c("time", "event")])
  DM <- cbind(data[, which(colnames(data) == "Race"):which(colnames(data) == "RVpowerIndex_S")], data[, c("time", "event")])
...
# New find_best_N_for_dataset function that first includes variables with more than 80% frequency, then gradually selects additional features
find_best_N_for_dataset <- function(reps, default_params, dataset, sorted_valid_features, mandatory_features, dataset_name) {
  ...
  selected_features <- mandatory_features # Start with mandatory features selected at more than 80% frequency
...
# Train and evaluate the model, add saving functionality
train_and_evaluate_models <- function(data, selected_features, best_params, predict_times, model_name, save_path) {
  cindex_results <- data.frame()
  brier_results <- data.frame()
  auc_results <- data.frame()
  iauc_values <- data.frame()
  roc_results <- data.frame()
  # Dynamically create an empty list variable with a suffix
  variable_name <- paste0("rsf_models_", model_name)
  assign(variable_name, list(), envir = .GlobalEnv)
  
  for (i in 1:25) {
    cat("Iteration:", i, "\n")
    results <- train_and_evaluate_rsf(data, model_name, best_params, selected_features, predict_times, iteration = i)
    
    # Assuming train_and_evaluate_rsf returns results including the model object
    current_models <- get(variable_name, envir = .GlobalEnv)
    current_models[[i]] <- results$model
    assign(variable_name, current_models, envir = .GlobalEnv)
    
    cindex_results <- rbind(cindex_results, data.frame(Model = model_name, CIndex = results$CIndex))
    brier_results <- rbind(brier_results, results$BrierScoreData)
    auc_results <- rbind(auc_results, results$AUCData)
    iauc_values <- rbind(iauc_values, data.frame(Model = model_name, iAUC = results$iAUC))
    roc_results <- rbind(roc_results, results$ROCData)  
  }
  
  # Save the model to the specified file path
  save(list = variable_name, file = save_path)
...
# Calculate and plot ROC curves for MAGGIC
calculate_and_plot_MAGGIC_ROC <- function(data, predict_times) {
  ...
  for (time in predict_times) {
    ...
    # Store raw ROC data
    roc_data <- rbind(roc_data, data.frame(Model = "MAGGIC", Time = time, FPR = roc_curve$FP, TPR = roc_curve$TP))
  }
  
  return(list(plots = plot_list, roc_data = roc_data))
}
...
save_results_to_excel <- function(auc_results, cindex_results, file_prefix) {
...
evaluate_existing_models <- function(rsf_model_list, predict_times, data, model_name) {
  ...
  for (model_idx in 1:length(rsf_model_list)) {
    ...
    for (predict_time in predict_times) {
      roc_curve <- survivalROC(Stime = data$time, status = data$event, marker = RSF.pred, predict.time = predict_time, method = "KM")
      roc_data <- rbind(roc_data, data.frame(Model = model_name, Time = predict_time, FPR = roc_curve$FP, TPR = roc_curve$TP))
    }
  }
  
  return(list(cindex_results = cindex_results, 
                brier_results = brier_results, 
                auc_results = auc_results, 
                iauc_values = iauc_values, 
                roc_data = roc_data))
