...
if (mode == 1) {
  ...
  # One issue I find problematic is the variability in RSF training results due to its inherent randomness. 
  # While the mandatory features remain consistent, the forward selection process is influenced by this randomness. 
  # For instance, the number of selected features can vary between 14 and 16. 
  # In the paper, we chose the feature set that yielded the best performance in downstream analysis. 
  # It's worth noting that the stepwise approach only slightly improves performance compared to variable hunting alone.
  Nresult_D <- find_best_N_for_dataset(10, default_params, subsets$D, sorted_valid_features_D, mandatory_features_D, "D")
  ...
  Nresult_M <- find_best_N_for_dataset(10, default_params, subsets$M, sorted_valid_features_M, mandatory_features_M, "M")
  ...
  Nresult_DM <- find_best_N_for_dataset(10, default_params, subsets$DM, sorted_valid_features_DM, mandatory_features_DM, "DM")
  ...
  # Summarize time dependent AUC data and plot
  ...
  # Save results to Excel
  save_results_to_excel(results_D$auc_results, results_D$cindex_results, "D")
  save_results_to_excel(results_M$auc_results, results_M$cindex_results, "M")
  save_results_to_excel(results_DM$auc_results, results_DM$cindex_results, "DM")
